...
- [2026-09-15 — PROD deploy plan v2: live-verified + checkpoints](sessions/revelator-bi-snowflake/2026-09-15-prod-deploy-plan-v2-and-checkpoints.md) — Rewrote the plan against live PROD state (all 6 repos' PRs merged, PROD SingleStore schema queried directly, every CI/CD workflow file read for the real deploy mechanism per repo incl. quango's missing slot-swap and develop/platform-vs-master gap); documented coordination needed with a colleague's unmerged UGC royalty-estimates feature sharing the same AGG tables; marked every step as a safe checkpoint or not, fixing a real step-ordering bug along the way; committed to a new branch + PR link given
